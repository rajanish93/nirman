    ...
    <div id="wpbody-content">
        ...
        <div class="wrap">
           ...
           <form action="<?php echo esc_html(admin_url('admin-post.php')); ?>" method="post"
               ...
               <input type="hidden" name="action" value="ct_admin_update">
               <?php wp_nonce_field('ct_admin_update', 'ct_admin'); ?>
                <div class="form-group">
                    <label for="inputAddress">Toper Name</label>
                    <input type="text" name="topername" class="form-control" id="inputAddress"
                        value="<?php echo $data['topername'];?>">
                    <input type="hidden" name="toperid" value="<?php echo $data['toperid'];?>">
                </div>
                <div id="newinput">
                    <div class="form-row" id="row">
                        <?php  foreach ($data['file'] as $value) {?>
                            <?php current($data['file'])===$value?$label='<label for="">File Name</label>':$label='';?>
                            <?php current($data['file'])===$value?$filelabel='<label for="">File</label>':$filelabel='';?>
                             <input type="hidden" value="<?php echo $value['file_id'];?>" name="file_id[]">
                            <div class="form-group col-md-5">
                                <?php echo $label;?>
                                <input type="text" value="<?php echo $value['file_name'];?>" class="form-control"
                                    id="inputEmail4" name="filename[]">
                            </div>
